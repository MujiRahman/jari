...
function ListPokemon() {
    ...
    const pilihPokemon = (data) => {
        ...
        console.log('isi data',stored_datas)
    }
    const masukFavorit = () => {
        if (suka) {
            const stored_datas = JSON.parse(localStorage.getItem("datas"));
            const remove = stored_datas.filter(hapus => hapus.id !== select.id)
            localStorage["datas"] = JSON.stringify(remove);
            setSuka(false)
        } else {
            const stored_datas = JSON.parse(localStorage.getItem("datas"));
            if (stored_datas) {
                const datas = [ ...stored_datas ,select];
    ...
    <div className='flex container'>
        {
            ...
            <div className='w-1/2 flex justify-center '>
                <div className='fixed top-1/2'>List Pokemon</div>
            </div> 
        }
        
        <div className={`w-1/2 flex flex-wrap gap-5 mt-28 ${detail ? 'ml-[53%]' : ''}`}>
            {
                loading ? <h1>Sedang Get Data...</h1> :
                <>
                {
                    state.map((pokemon) => {
                    return(
